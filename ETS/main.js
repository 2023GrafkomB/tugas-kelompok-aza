...
class BasicWorldDemo {
    constructor() {
        ...
        document.getElementById("game-menu").onclick = (msg) =>
        this._OnStart(msg);
      }
    ...
    _Initialize() {
        ...
        this.scene_.fog = new THREE.FogExp2(0x89b2eb, 0.00125);
           ...
           const groundTexture = textureLoader.load('./resources/DesertPack/Blend/Textures/dusted.jpg');
          
           groundTexture.wrapS = groundTexture.wrapT = THREE.RepeatWrapping;
           groundTexture.repeat.set(500, 500); // Repeat the texture 20 times on both axes
     
           const groundMaterial = new THREE.MeshStandardMaterial({
               map: groundTexture,
               // color: 0xffffff, // Set a base color for the ground
               roughness: 1, // Adjust roughness for a more realistic surface
           });
           ...
           let animationRunning = true; // Boolean variable to track if the animation is running
           
           function animateTexture(timeElapsed) {
               if (!animationRunning) {
                   return; // If animationRunning is false, stop the animation
               }
           
               // Calculate the texture movement based on time elapsed
               textureOffsetX -= timeElapsed * -0.0003; // Adjust the speed of the movement by changing this value
               groundTexture.offset.set(textureOffsetX, 1);
               if (textureOffsetX < -1) {
                   textureOffsetX = 1;
               }
           }
           ...
           function animate() {
               if (!animationRunning) {
                   return; // If animationRunning is false, stop calling requestAnimationFrame
               }
           
               requestAnimationFrame(animate);
               const currentTime = performance.now();
               const timeElapsed = currentTime - prevTime;
               animateTexture(timeElapsed);
           ...
           function stopAnimation() {
               animationRunning = false; // Set the animationRunning variable to false to stop the animation
           }
        ...
        this.background_ = new background.Background({
            scene: this.scene_,
        });
